...
class VoiceFallbackReplyTest {
    ...
    @Test
    void voiceMessageSendsTextFallbackAfterProbe() throws Exception {
        ...
        when(client.downloadMedia("encrypt-query", "aes-key")).thenReturn(new byte[]{1, 2, 3});

        ReflectionTestUtils.invokeMethod(botService, "processVoiceProbe", "wx-user", "ctx-token", voice);

        verify(client, timeout(1000)).sendTextMessage(any(LoginCredentials.class), eq("wx-user"),
                eq("ctx-token"), eq("暂时无法识别语音，请发送文字"));
    }
